#############################################################################
##
#W  subgroup.g              SubgroupLattice library            Manuel Martins
##
#Y  Copyright (C) 2017 Manuel Martins
##
## This is an example of the possible usage of Francy to produce
## interactive content
##
...
if not IsBound(IsGraphicGraphRep) then
  DeclareRepresentation( "IsGraphicGraphRep",
    IsComponentObjectRep and IsAttributeStoringRep and IsCanvas and
    IsCanvasRep,
...
if not IsBound(IsGraphicPosetRep) then
  DeclareRepresentation( "IsGraphicPosetRep",
        ...
        IsCanvas and IsCanvasRep and IsGraphicGraphRep,
# we inherit those components from the sheet:
    [ "name", "width", "height", "gapMenu", "callbackName", "callbackFunc",
      "menus", "objects", "free",
# now our own components:
...
if not IsBound(IsGraphicSubgroupLattice) then
  DeclareRepresentation( "IsGraphicSubgroupLattice",
    IsComponentObjectRep and IsAttributeStoringRep and IsCanvas and
    IsCanvasRep and IsGraphicGraphRep and IsGraphicPosetRep,
# we inherit those components from the sheet:
    [ "name", "width", "height", "gapMenu", "callbackName", "callbackFunc",
      "menus", "objects", "free",
...
function(G, def)
  ...
  poset := Canvas(CanvasType!.HASSE, defaults.title);
  filters := G;
...
#
  ...
  if latticetype[4] then
    ...
    v1 := Shape(ShapeType!.CIRCLE, "1");
    poset!.add(v1);

    # connect the two vertices
    poset!.add(Link(v1, v2));
...
function( name, width, height )
  ...
  poset :=  Canvas(CanvasType!.HASSE, name);
